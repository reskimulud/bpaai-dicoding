...
class RandomNumberWidget : AppWidgetProvider() {
    ...
    private fun updateAppWidget(
        ...
        val views = RemoteViews(context.packageName, R.layout.random_number_widget)
        views.setTextViewText(R.id.appwidget_text, lastUpdate)
        views.setOnClickPendingIntent(R.id.btn_click, getPendingIntentSelf(context, appWidgetId, WIDGET_CLICK))

        // Instruct the widget manager to update the widget
        appWidgetManager.updateAppWidget(appWidgetId, views)
    }

    private fun getPendingIntentSelf(context: Context, appWidgetId: Int, action: String): PendingIntent {
